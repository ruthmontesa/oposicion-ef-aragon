...
function bloqueLabel(n: number) {
  return n <= 10 ? 'A' : n <= 20 ? 'B' : 'C'
}

function cardClasses(p: number | null, v: number) {
  if (v === 0 || p === null) return 'border-gray-200 bg-gray-50 hover:border-gray-300'
  if (p >= 70) return 'border-brand-200 bg-brand-50 hover:border-brand-300'
  if (p >= 40) return 'border-yellow-200 bg-yellow-50 hover:border-yellow-300'
  return 'border-red-200 bg-red-50 hover:border-red-300'
}

function scoreClasses(p: number | null, v: number) {
  if (v === 0 || p === null) return 'text-gray-400'
  if (p >= 70) return 'text-brand-600 font-semibold'
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        <section>
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Secciones</h2>
        ...
        <section>
          <div className="flex items-baseline justify-between mb-4">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Temario — 25 temas</h2>
            <div className="flex gap-3 text-[10px] text-gray-400">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-brand-400 inline-block" />&gt;70%</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-yellow-400 inline-block" />40-70%</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-gray-300 inline-block" />Sin empezar</span>
            </div>
          </div>
            ...
            {temas.map(({ numero, titulo, puntuacion, vecesEstudiado }) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-gray-500 tabular-nums">T{String(numero).padStart(2, '0')}</span>
                  <span className="text-[9px] font-semibold text-gray-400 bg-white/80 rounded px-1 py-0.5">Blq. {bloqueLabel(numero)}</span>
                </div>
                <p className="text-xs text-gray-700 leading-snug line-clamp-3 flex-1">{titulo}</p>
                <p className={`text-xs ${scoreClasses(puntuacion, vecesEstudiado)}`}>
                  {vecesEstudiado === 0 ? 'Sin empezar' : puntuacion !== null ? `${Math.round(puntuacion)}%` : 'Estudiado'}
                </p>
              </Link>
            ))}
          </div>
